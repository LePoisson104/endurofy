...
export const useManualExerciseSets = (
  ...
  useEffect(() => {
    if (workoutLogResponse?.data?.[0]?.workoutExercises) {
      ...
      const loggedExercises = workoutLogResponse?.data?.[0]?.workoutExercises
        ? groupWorkoutLogByExercise(
            workoutLogResponse?.data?.[0]?.workoutExercises
          )
        : {};

      workoutLogResponse?.data?.[0]?.workoutExercises.forEach(
        (workoutExercise: any) => {
  ...
  }, [workoutLogResponse, exerciseMap]); // Only include stable dependencies
  ...
  const getWorkoutExerciseId = useCallback(
    (exerciseId: string): string => {
      return (
        workoutLogResponse?.data?.[0]?.workoutExercises.find(
  ...
  const getExerciseNotes = useCallback(
    (workoutExerciseId: string): string => {
      const notes = workoutLogResponse?.data?.[0]?.workoutExercises.find(
  ...
  const getExercises = useCallback((): Exercise[] => {
    if (!workoutLogResponse?.data?.[0]?.workoutExercises) return [];

    return workoutLogResponse?.data?.[0]?.workoutExercises.map(
      (workoutExercise: any) =>
        convertWorkoutExerciseToExercise(workoutExercise)
    );
  }, [workoutLogResponse, convertWorkoutExerciseToExercise]);
